...
for g = 1 : length(orderPlot)
    ...
    c = 1;
    for p = orderPlot{g}
        imgsize = size(imread(sprintf('%s/rms_HPT_chan_%d.tif', dir.figFolder, p))); % get image size
        width = [num2str(1.15 * imgsize(2)/imgsize(1)) 'in'];
        images{p} = Image(sprintf('%s/rms_HPT_chan_%d.tif', dir.figFolder, p)); % read images from folder
